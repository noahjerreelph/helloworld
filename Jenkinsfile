pipeline {
    ...
    stages {
        stage('Test') {
            when { branch "master" }
            steps {
                 sshCommand remote: env.remote, command: 'cd /var/www/helloworld/helloworld_staging && sudo git pull origin master'
                 sshCommand remote: env.remote, command: 'uid=$(forever list | grep /var/www/hello_world/helloworld_staging/index.js  | cut -c24-27) && forever stop $uid'
                 sshCommand remote: env.remote, command: 'NODE_ENV=staging forever start /var/www/hello_world/helloworld_staging/index.js'
            }
        }
        stage('Release') {
            when { tag "*" }
